...
def numero_por_extenso(numero):
	...
	for n in [n1, n2]:
		...
		if cent != 0:
			...
			# Adiciona mil caso ja exista valor (segundo loop)
			if retorno_extenso:
				retorno_extenso += ' Mil, '

			retorno_extenso += "%s" % extenso_list[3][cent]
		...
		if dez == 1:
			...
			retorno_extenso += "%s" % extenso_list[1][und]
			...
			if dez != 0:
				...
				retorno_extenso += "%s" % extenso_list[2][dez]

			if und != 0:
				if retorno_extenso:
					retorno_extenso += " e "

				retorno_extenso += "%s" % extenso_list[0][und]
